-- Idempotent replay of scripts/schema.sql. On a fresh docker volume the
-- initdb mount has already run schema.sql, so every statement here no-ops;
-- on an existing volume (or a bare local Postgres) this brings the core
-- schema up without touching data. The one deliberate difference from
-- schema.sql: the materialized view is only created when missing instead of
-- dropped and rebuilt, so an already-populated view survives app restarts.
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_matviews WHERE matviewname = 'hourly_occupancy'
    ) THEN
